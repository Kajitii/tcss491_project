...
TileMap.prototype.generateMap = function () {
    ...
    //var n = Math.round(10 + Math.random() * 20);
    //var o = n + Math.round(n * 0.4 * (-0.5 + Math.random()));
    //this.mapData = new Array(n);
    //var factor = 1;
    //var gen = new ClassicalNoise();
    //for (var j = 0; j < this.mapData.length; j++) {
    //    this.mapData[j] = new Array(o);
    //    for (var i = 0; i < this.mapData[0].length; i++) {
    //        var m = gen.noise(i * factor, j * factor, 0.5);
    //        if (m > 0.0) this.mapData[j][i] = 0x10000;
    //        else if (m > -0.2) this.mapData[j][i] = 0x20000;
    //    }
    //}
}
